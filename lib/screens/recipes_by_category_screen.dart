...
class _RecipesByCategoryScreenState extends State<RecipesByCategoryScreen> {
  ...
  Future<void> _onSearch(String q) async {
    ...
    // The API search endpoint is global; filter results to the selected category.
    setState(() => _loading = true);
    try {
      final results = await api.searchRecipes(query);
      final filtered = results.where((m) => m.strRecipe.isNotEmpty).toList();
      // But we must ensure same category: need to check detail for each result or cross-check - simpler: fetch details for each and filter by category.
      // To avoid many network calls, we'll filter by checking if meal exists in the _meals list (by id) — since fetchMealsByCategory returns meals in this category.
